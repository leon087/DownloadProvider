...
import static cm.android.download.providers.downloads.DownloadManager.COLUMN_LOCAL_FILENAME;
import static cm.android.download.providers.downloads.DownloadManager.COLUMN_LOCAL_URI;
import static cm.android.download.providers.downloads.DownloadManager.COLUMN_MEDIA_TYPE;
import static cm.android.download.providers.downloads.DownloadManager.COLUMN_URI;

import java.io.File;

import cm.android.download.provider.Downloads.Impl.RequestHeaders;
...
public class OpenHelper {
	...
	public static Intent buildViewIntent(Context context, long id) {
		final DownloadManager downManager = new DownloadManager(
		...
		try {
			...
			if ("application/vnd.android.package-archive".equals(mimeType)) {
				...
				// FIXME, what do these do for
				// intent.putExtra(Intent.EXTRA_REFERRER, getRefererUri(context,
				// id));
				...
				intent.setDataAndType(ContentUris.withAppendedId(
						ALL_DOWNLOADS_CONTENT_URI, id), mimeType);
			} else {
				intent.setDataAndType(localUri, mimeType);
			}
	...
	private static Uri getRefererUri(Context context, long id) {
		...
		final Cursor headers = context.getContentResolver().query(headersUri,
				null, null, null, null);
		try {
			while (headers.moveToNext()) {
				final String header = getCursorString(headers,
						RequestHeaders.COLUMN_HEADER);
	...
	private static int getOriginatingUid(Context context, long id) {
		final Uri uri = ContentUris.withAppendedId(ALL_DOWNLOADS_CONTENT_URI,
				id);
			...
			try {
				if (cursor.moveToFirst()) {
					return cursor.getInt(cursor
							.getColumnIndexOrThrow(Constants.UID));
				}
			} finally {
				cursor.close();
			}
